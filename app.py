...
def generate_json_output(structured_data, analytics):
    output = {"structured_data": structured_data, "analytics": analytics}
...
def main():
    ...
    if not SPACY_AVAILABLE:
        st.warning(
            "spaCy is not installed. Some advanced text analysis features will be limited."
        )
    if not DOCX_AVAILABLE:
        st.warning(
            "python-docx is not installed. DOCX file processing will not be available."
        )
    if not PDF_AVAILABLE:
        st.warning(
            "PyPDF2 is not installed. PDF file processing will not be available."
        )
