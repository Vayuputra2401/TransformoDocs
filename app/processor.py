...
nlp = spacy.load(
    "en_core_web_trf"
)  # Use the transformer-based spaCy model for better performance
...
def apply_template(data, template):
    if template == "data_only":
        ...
        return {
            "persons": [
                ent["text"] for ent in data["entities"] if ent["label"] == "PERSON"
            ],
            "organizations": [
                ent["text"] for ent in data["entities"] if ent["label"] == "ORG"
            ],
            "locations": [
                ent["text"]
            ...
            "dates": [
                ent["text"] for ent in data["entities"] if ent["label"] == "DATE"
            ],
            "money": [
                ent["text"] for ent in data["entities"] if ent["label"] == "MONEY"
            ],
            "percent": [
                ent["text"] for ent in data["entities"] if ent["label"] == "PERCENT"
            ],
            "time": [ent["text"] for ent in data["entities"] if ent["label"] == "TIME"],
            "quantity": [
                ent["text"] for ent in data["entities"] if ent["label"] == "QUANTITY"
            ],
            "ordinal": [
                ent["text"] for ent in data["entities"] if ent["label"] == "ORDINAL"
            ],
            "cardinal": [
                ent["text"] for ent in data["entities"] if ent["label"] == "CARDINAL"
            ],
        }
    else:
        return data
...
def analyze_document(structured_data, full_text):
    analytics = {
        ...
        "average_sentence_length": (
            round(structured_data["word_count"] / structured_data["sentence_count"], 2)
            if structured_data["sentence_count"] > 0
            else 0
        ),
    ...
    doc = nlp(full_text)
    analytics["most_common_entities"] = Counter(
        [ent["label"] for ent in structured_data["entities"]]
    ).most_common(5)
    analytics["most_common_words"] = Counter(
...
def process_document(
    extracted_text, template=None, custom_fields=None, candidate_labels=None
):
    warnings = []
    ...
    if candidate_labels:
        output_data["classification"] = classify_document(
            extracted_text, candidate_labels
        )
