...
class EnhancedJSONAgent:
    ...
    def _prepare_prioritized_context(self, query):
        ...
        # Prioritize "words" and "sentences" fields
        prioritized_items = [item for item in context_items if "words" in item or "sentences" in item]
        ...
        def find_names(data):
            ...
            if isinstance(data, dict):
                for key, value in data.items():
                    if any(name_key in key.lower() for name_key in ['name', 'nome', 'full_name', 'fullname']):
    ...
    def _extract_contacts(self, query):
        ...
        def find_contacts(data):
            ...
            if isinstance(data, dict):
                for key, value in data.items():
                    if any(contact_key in key.lower() for contact_key in ['contact', 'phone', 'email', 'telephone']):
